...
(function($, undefined) {
    ...
    $.fn.split = function(options) {
        ...
        var settings = $.extend({
            ...
            position: '50%',
            reverse: false, // Custom-XBase-DucNV: if reverse position, set css from left to right & top to bottom
            invisible: false,
            onDragStart: $.noop,
            onDragEnd: $.noop,
            onDrag: $.noop
        ...
        var parent = this.closest('div[class*="_panel"]');
        if (parent.length) {
        ...
        var splitter = $('<div/>').addClass(cls)
        .append('<div class="' + cls + '-custom"></div>') // Custom-XBase-DucNV: for beauty thin splitter with wide width for easy mouse hover resizing
                        ...
                        if (n === undefined) {
                            ...
                            if (settings.invisible) {
                                ...
                                if (!settings.reverse) {
                                    ...
                                    panel_1.width(self.width());
                                    panel_2.width(position);
                                }
                                // Custom-XBase-DucNV
                        ...
                        if (n === undefined) {
                            ...
                            if (settings.invisible) {
                                ...
                                if (!settings.reverse) {
                                    ...
                                    panel_1.height(self.height() - position);
                                    panel_2.height(position);
                                }
                                if (!settings.reverse)
                                    splitter.css('top', pw - sw2);
                                else
                                    splitter.css('bottom', position);
                            } else {
                                // Custom-XBase-DucNV
                                ...
                                if (!settings.reverse) {
                                    ...
                                    panel_1.height(self.height() - position - sw);
                                    panel_2.height(position);
                                }
                                // Custom-XBase-DucNV
                                //splitter.css('top', pw);
        ...
        self.bind('splitter.resize', function(e) {
            ...
            self.position(!self.reverse ? pos : (self.orientation == 'vertical' ? self.width() - pos : self.height() - pos), true);
        });
