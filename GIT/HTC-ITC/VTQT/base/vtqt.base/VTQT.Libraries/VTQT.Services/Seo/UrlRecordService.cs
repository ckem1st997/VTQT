...
namespace VTQT.Services.Seo
{
    ...
    public partial class UrlRecordService : IUrlRecordService
    {
        ...
        {
            _urlRecordRepository = EngineContext.Current.Resolve<IRepository<UrlRecord>>(DataConnectionHelper.ConnectionStringNames.Master);
        ...
        protected virtual async Task UpdateUrlRecordAsync(UrlRecord urlRecord)
        {
            ...
            await _cacheManager.HybridProvider.RemoveByPrefixAsync(SeoCacheKeys.UrlRecordAllPrefix);
        }
        ...
        /// </returns>
        public virtual async Task<IList<UrlRecord>> GetUrlRecordsByIdsAsync(string[] urlRecordIds)
        {
            return await _urlRecordRepository.GetByIdsAsync(urlRecordIds);
        }
        ...
        public virtual async Task<UrlRecord> GetBySlugAsync(string slug)
        {
            ...
            var key = SeoCacheKeys.UrlRecordBySlugCacheKey.FormatWith(slug);
            ...
            var urlRecord = (await _cacheManager.HybridProvider.GetAsync(key, async () =>
        ...
        public virtual async Task<IPagedList<UrlRecord>> GetAllUrlRecordsAsync(
            string slug = "", string languageId = null, bool? isActive = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            // TODO-XBase-Recheck
            ...
            var key = SeoCacheKeys.UrlRecordAllCacheKey;

            var entities = (await _cacheManager.HybridProvider.GetAsync(key, async () =>
            {
                return await _urlRecordRepository.Table.OrderBy(ur => ur.Slug).ToListAsync();
            }, TimeSpan.FromMinutes(CachingDefaults.MonthCacheTime))).Value;

            var result = Enumerable.Empty<UrlRecord>();
            if (!string.IsNullOrWhiteSpace(slug))
                result = entities.Where(ur => ur.Slug.Contains(slug));

            if (!string.IsNullOrWhiteSpace(languageId))
                result = entities.Where(ur => ur.LanguageId == languageId);

            if (isActive.HasValue)
                result = entities.Where(ur => ur.IsActive == isActive);

            return new PagedList<UrlRecord>(result, pageIndex, pageSize, entities.Count);
        }
        ...
        /// </returns>
        public virtual async Task<string> GetActiveSlugAsync(string entityId, string entityName, string languageId)
        {
            //gradual loading
            var key = SeoCacheKeys.UrlRecordCacheKey.FormatWith(entityId, entityName, languageId);
            ...
            var rezSlug = (await _cacheManager.HybridProvider.GetAsync(key, async () =>
                await query.FirstOrDefaultAsync()
                , TimeSpan.FromMinutes(CachingDefaults.MonthCacheTime))).Value
                ?? string.Empty;
        ...
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task SaveSlugAsync<T>(T entity, string slug, string languageId) where T : BaseEntity, ISlugSupported
        {
            if (entity == null)
        ...
        /// </returns>
        public virtual async Task<string> GetSeNameAsync<T>(T entity, string languageId = null, bool returnDefaultValue = true,
            bool ensureTwoPublishedLanguages = true) where T : BaseEntity, ISlugSupported
        {
            ...
            return await GetSeNameAsync(entity.Id, entityName, languageId ?? _workContext.WorkingLanguage.Id, returnDefaultValue, ensureTwoPublishedLanguages);
        }
        ...
        /// The task result contains the search engine  name (slug)
        /// </returns>
        public virtual async Task<string> GetSeNameAsync(string entityId, string entityName, string languageId = null,
            bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
        {
            ...
            if (!string.IsNullOrWhiteSpace(languageId))
            {
                ...
                if (loadLocalizedValue)
                    result = await GetActiveSlugAsync(entityId, entityName, languageId);
            }

            //set default value if required
            if (string.IsNullOrEmpty(result) && returnDefaultValue)
                result = await GetActiveSlugAsync(entityId, entityName, SettingDefaults.AllLanguagesId);
        ...
        /// The task result contains the valid sename
        /// </returns>
        public virtual async Task<string> ValidateSeNameAsync(string entityId, string entityName, string seName, string name, bool ensureNotEmpty)
        {
