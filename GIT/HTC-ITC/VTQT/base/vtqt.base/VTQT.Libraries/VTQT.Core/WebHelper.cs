...
namespace VTQT.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetAppLocation(bool? useSsl = null)
        {
            ...
            //if host is empty (it is possible only when HttpContext is not available), use URL of a app entity configured in admin area
            if (string.IsNullOrEmpty(appHost))
            {
                //do not inject IWorkContext via constructor because it'll cause circular references
                appLocation = EngineContext.Current.Resolve<IAppContext>().CurrentApp?.Url
                    ?? throw new Exception("Current app cannot be loaded");
            }
