...
namespace VTQT.Web.Framework.UI
{
    ...
    public partial class PageHeadBuilder : IPageHeadBuilder
    {
        ...
        private readonly IXBaseFileProvider _fileProvider;
        ...
        public PageHeadBuilder(
            ...
            IXBaseFileProvider fileProvider,
        ...
        public virtual string GenerateScripts(ResourceLocation location, bool? bundleFiles = null)
        {
            ...
            if (bundleFiles.Value)
            {
                ...
                if (partsToBundle.Any())
                {
                    ...
                    //var cacheKey = new CacheKey($"XBase.minification.shouldrebuild.js-{outputFileName}")
                    //{
                    //    CacheTime = CachingDefaults.BundledFilesCacheTime
                    //};
                    var cacheKey = $"XBase.minification.shouldrebuild.js-{outputFileName}";

                    //var shouldRebuild = _memoryCache.GetAsync(_staticCacheManager.PrepareKey(cacheKey), () => true).Result;
                    var shouldRebuild = _memoryCache.GetOrCreate(cacheKey, entry => true);
                    ...
                    if (shouldRebuild)
                    {
                        ...
                        _memoryCache.Set(cacheKey, false, TimeSpan.FromMinutes(CachingDefaults.BundledFilesCacheTime));
                    }
        ...
        public virtual string GenerateCssFiles(ResourceLocation location, bool? bundleFiles = null)
        {
            ...
            if (bundleFiles.Value)
            {
                ...
                if (partsToBundle.Any())
                {
                    ...
                    //we periodically re-check already bundles file
                    //so if we have minification enabled, it could take up to several minutes to see changes in updated resource files (or just reset the cache or restart the site)
                    //var cacheKey = new CacheKey($"XBase.minification.shouldrebuild.css-{outputFileName}")
                    //{
                    //    CacheTime = CachingDefaults.BundledFilesCacheTime
                    //};
                    var cacheKey = $"XBase.minification.shouldrebuild.css-{outputFileName}";
