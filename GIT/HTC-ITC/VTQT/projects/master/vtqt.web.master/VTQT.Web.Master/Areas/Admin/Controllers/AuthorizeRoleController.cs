...
{
    [XBaseMvcAuthorize]
    public class AuthorizeRoleController : AdminMvcController
    {
        #region Fields
        ...
        {
            var res = await ApiHelper.ExecuteAsync<RoleModel>("/authorize-role/authorize",
                new
                {
                    roleId = roleId
        ...
        public async Task<IActionResult> Authorize(AuthorizeRoleModel model)
        {
            ...
            var res = await ApiHelper.ExecuteAsync("/authorize-role/authorize", model, Method.POST, ApiHosts.Master);
            if (!res.success)
            {
        ...
        public async Task<IActionResult> Get([DataSourceRequest] DataSourceRequest request, RoleSearchModel searchModel)
        {
            ...
            var res = await ApiHelper.ExecuteAsync<List<RoleModel>>("/authorize-role/get", searchModel, Method.GET, ApiHosts.Master);
            var data = res.data;
        ...
        {
            var res = await ApiHelper.ExecuteAsync<IList<AppActionTreeModel>>("/authorize-role/get-app-action-tree", new
            {
                appId = appId,
