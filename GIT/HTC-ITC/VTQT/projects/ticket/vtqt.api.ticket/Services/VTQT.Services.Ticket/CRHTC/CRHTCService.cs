...
{
    public class CRHTCService : ICRHTCService
    {
        #region Fields
        ...
        private readonly IRepository<Core.Domain.Ticket.CR_HTC> _crHtcRepository;
        ...
        public CRHTCService()
        {
            ...
            _crHtcRepository = EngineContext.Current.Resolve<IRepository<CR_HTC>>(DataConnectionHelper.ConnectionStringNames.Ticket);
        }
        ...
        public async Task<int> InsertAsync(CR_HTC entity)
        {
            if (entity == null)
            {
            ...
            var result = await _crHtcRepository.InsertAsync(entity);

            return result;
        }

        public async Task<int> UpdateAsync(CR_HTC entity)
        {
            if (entity == null)
            {
                throw new ArgumentNullException(nameof(entity));
            ...
            var result = await _crHtcRepository.UpdateAsync(entity);
        ...
        public async Task<int> DeletesAsync(IEnumerable<string> ids)
        {
            ...
            var result = await _crHtcRepository.DeleteAsync(ids);

            return result;
        }

        public async Task<CR_HTC> GetByIdAsync(string id)
        {
            ...
            var result = await _crHtcRepository.GetByIdAsync(id);

            return result;
        }

        public async Task<CR_HTC> GetByCRIdAsync(string crId)
        {
            ...
            var result = await _crHtcRepository.Table
        ...
        public async Task<IPagedList<CRHTCGridModel>> Get(CRHTCSearchContext ctx)
        {
            ctx.Keywords = ctx.Keywords?.Trim();

            var query = from p in _crHtcRepository.Table
            ...
            return new PagedList<CRHTCGridModel>(query, ctx.PageIndex, ctx.PageSize);
        }

        public IList<CR_HTC> GetByCRHTCId(CRHTCSearchContext ctx)
        {
            if (string.IsNullOrWhiteSpace(ctx.CrId))
                throw new ArgumentNullException(nameof(ctx.CrId));

            var query =
                from x in _crHtcRepository.Table
