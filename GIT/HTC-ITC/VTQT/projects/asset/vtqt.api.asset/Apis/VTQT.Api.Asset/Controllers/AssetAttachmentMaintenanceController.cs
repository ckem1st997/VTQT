...
{
    [Route("attachment-maintenance")]
    [ApiController]
    [Produces("application/json")]
    [XBaseApiAuthorize]
    [AppApiController("Asset.Controllers.AssetAttachmentMaintenance")]
    public class AssetAttachmentMaintenanceController : AdminApiController
    {
        #region Fields
        ...
        #region Ctor
        public AssetAttachmentMaintenanceController(
        ...
        [Route("index")]
        [HttpGet]
        [AppApiAction("Asset.AppActions.AssetAttachmentMaintenances.Index")]
        ...
        public async Task<IActionResult> Create(IEnumerable<string> ids)
        {
            ...
            if (ids?.Count() > 0)
            {
                var assetAttachments = _assetAttachmentService.GetByIdsAsync(ids);
                if (assetAttachments?.Count > 0)
                {
                    assetAttachments.ToList().ForEach(x =>
                    {
        ...
        public async Task<IActionResult> Create(MaintenanceModel model)
        {
            ...
            entity.MaintenancedDate = model.MaintenancedDate.ToUniversalTime();
            ...
            if (model.MaintenanceDetails?.Count > 0)
            {
                model.MaintenanceDetails.ToList().ForEach(async x =>
                {
                    ...
                    var history = new History
                    {
                        ...
                        AssetAttachmentId = x.AssetAttachmentId,
                    ...
                    var assetAttachment = await _assetAttachmentService.GetByIdAsync(x.AssetAttachmentId);
                    assetAttachment.ModifiedBy = entity.EmployeeId;
                    assetAttachment.ModifiedDate = DateTime.UtcNow;
                    assetAttachment.MaintenancedDate = entity.MaintenancedDate;
        ...
        public async Task<IActionResult> DetailCreate([FromQuery] MaintenanceDetailSearchModel searchModel)
        {
            ...
            var entities = _maintenanceDetailService.GetAssetAttachments(searchContext);
