...
{
    [XBaseMvcAuthorize]
    public class AssetInfrastructorController : AdminMvcController
    {
        #region Fields
        ...
        #region Ctor
        public AssetInfrastructorController(IWorkContext workContext)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            Task getStations = AssetHelper.GetAvailableStation(model);
            var res = await ApiHelper
                .ExecuteAsync<AssetInfrastructorModel>("/asset-infrastructor/create", null, Method.GET, ApiHosts.Asset);
            var resLastSelected = await ApiHelper.
                ExecuteAsync<string>($"/organization/get-last-selected/?appId=6&userId={_workContext.UserId}&path=/Admin/AssetInfrastructor", null, Method.GET, ApiHosts.Master);

            await Task.WhenAll(new List<Task> { getCategories, getDurations, getStations });
        ...
        public async Task<IActionResult> Create(AssetInfrastructorModel model)
        {
            ...
            var res = await ApiHelper
                .ExecuteAsync("/asset-infrastructor/create", model, Method.POST, ApiHosts.Asset);
            if (!res.success)
            {
        ...
        {
            var res = await ApiHelper
                .ExecuteAsync<AssetInfrastructorModel>("/asset-infrastructor/edit", new { id = id }, Method.GET, ApiHosts.Asset);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(AssetInfrastructorModel model)
        {
            ...
            var res = await ApiHelper
                .ExecuteAsync("/asset-infrastructor/edit", model, Method.POST, ApiHosts.Asset);
            if (!res.success)
            {
                NotifyError(res.GetErrorsToHtml());
                return Ok(new XBaseResult { success = false });
        ...
        {
            var res = await ApiHelper
                .ExecuteAsync<AssetInfrastructorModel>("/asset-infrastructor/details", new { id = id }, Method.GET, ApiHosts.Asset);
            if (!res.success)
            {
                NotifyError(res.GetErrorsToHtml());
                return Ok(new XBaseResult { success = false });
        ...
        public async Task<IActionResult> Deletes(IEnumerable<string> ids)
        {
            ...
            var res = await ApiHelper
                .ExecuteAsync("/asset-infrastructor/deletes", ids, Method.POST, ApiHosts.Asset);
            if (!res.success)
            {
                NotifyError(res.GetErrorsToHtml());
                return Ok(new XBaseResult { success = false });
                                             ...
                                             AssetSearchModel searchModel)
        {
            ...
            var res = await ApiHelper
                .ExecuteAsync<List<AssetInfrastructorModel>>("/asset-infrastructor/get", searchModel, Method.GET, ApiHosts.Asset);
            var data = res.data;

            if (!string.IsNullOrEmpty(searchModel.OrganizationId))
            {
                var resLastSelected = await ApiHelper
                    .ExecuteAsync<string>($"/organization/update-last-selected/?appId=6&userId={_workContext.UserId}&path=/Admin/AssetInfrastructor&departmentId={searchModel.OrganizationId}", null , Method.POST, ApiHosts.Master);
            }
        ...
        {
            searchModel.BindRequest(request);
            searchModel.AssetTypeMaintenanceDetail = AssetType.Infrastructure;
        ...
        {
            var res = await ApiHelper.
                ExecuteAsync<AssetTransferenceModel>($"/asset-transference/create?assetType={(int)AssetType.Infrastructure}&id={id}", null, Method.GET, ApiHosts.Asset);
        ...
        public async Task<string> GetCategoryIdByAssetId(string id)
        {
            ...
            var res = await ApiHelper.ExecuteAsync<AssetInfrastructorModel>("/asset-infrastructor/get-by-id", new { id = id }, Method.GET, ApiHosts.Asset);
        ...
        public async Task<string> GetDepartmentAndEmployeeByAsset(string assetId)
        {
            ...
            var res = await ApiHelper.ExecuteAsync<AssetInfrastructorModel>("/asset-infrastructor/get-by-id", new { id = assetId }, Method.GET, ApiHosts.Asset);
        ...
        public async Task<IActionResult> GetCategories(string search, int page)
        {
            var res = await ApiHelper.ExecuteAsync<List<AssetCategoryModel>>(
                $"/asset-category/get-available?assetType={(int)AssetType.Infrastructure}", null, Method.GET, ApiHosts.Asset);
        ...
        /// <summary>
        /// Khởi tạo dữ liệu trạm
        /// </summary>
        /// <param name="code"></param>
        /// <returns></returns>
        public async Task<IActionResult> GetStation(string code)
        {
            var res = await ApiHelper
                .ExecuteAsync<StationModel>("/station/get-by-code", new { code = code }, Method.GET, ApiHosts.Master);
        ...
        public async Task<IActionResult> GetExcelData(AssetSearchModel searchModel)
        {
            ...
            var res = await ApiHelper
                .ExecuteAsync<List<AssetInfrastructorModel>>("/asset-infrastructor/get-excel", searchModel, Method.GET, ApiHosts.Asset);

            var data = res.data;

            if (data?.Count > 0)
            {
                var assetInfrastructor = new DataTable();
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.Stt), typeof(int));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.AllocationDate), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.Code), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.Name), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationName), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationCategory), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationCode), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationArea), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationLongitude), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationLatitude), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.StationAddress), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.CategoryName), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.OriginQuantity), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.RecallQuantity), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.BrokenQuantity), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.SoldQuantity), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.BalanceQuantity), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.UnitName), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.OrganizationUnitName), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.CurrentUsageStatus), typeof(string));
                assetInfrastructor.Columns.Add(nameof(AssetExcelModel.MaintenancedDate), typeof(string));

                var info = new DataTable();
                info.Columns.Add("title", typeof(string));

                info.Rows.Add("Danh sách tài sản hạ tầng");
                info.TableName = "info";
                var stt = 1;
                data.ForEach(x =>
                {
                    ...
                    if (!string.IsNullOrEmpty(x.CategoryId) && !string.IsNullOrEmpty(x.CurrentUsageStatus))
                    {
                        ...
                        Task<XBaseResult<Station>> getStation = ApiHelper.ExecuteAsync<Station>("station/get-by-code", new { code = x.StationCode }, Method.GET, ApiHosts.Asset);

                        Task.WhenAll(new List<Task> { getCategory, getUsageStatus, getStation });

                        var category = getCategory.Result.data;
                        var usage = getUsageStatus.Result.data;
                        var station = getStation.Result.data;

                        m.CategoryName = category?.Name;
                        m.CurrentUsageStatus = usage?.Description;
                        m.StationAddress = station?.Address;
                        m.StationArea = station?.Area;
                        m.StationCategory = station?.Note;
                        m.StationLatitude = station?.Latitude ?? 0;
                        m.StationLongitude = station?.Longitude ?? 0;
                    }

                    assetInfrastructor.Rows.Add(m.Stt, m.AllocationDate, m.Code,
                                         m.Name, m.StationName, m.StationCategory,
                                         m.StationCode, m.StationArea, m.StationLongitude,
                                         m.StationLatitude, m.StationAddress, m.CategoryName, 
                                         m.OriginQuantity, m.RecallQuantity, m.BrokenQuantity,
                                         m.SoldQuantity, m.BalanceQuantity, m.UnitName,
                                         m.OrganizationUnitName, m.CurrentUsageStatus, m.MaintenancedDate);
                });
                assetInfrastructor.TableName = "assetInfrastructor";
                var ds = new DataSet();
                ds.Tables.Add(assetInfrastructor);
                ds.Tables.Add(info);
                var tmpPath = CommonHelper.MapPath("/wwwroot/Teamplate/Excel/AssetInfrastructor.xlsx");
                var bytes = TemplateExcel.FillReport("AssetInfrastructor.xlsx", tmpPath, ds, new string[] { "{", "}" });

                return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "ds_taisanhatang.xlsx");
            }
